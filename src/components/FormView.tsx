...
export default function FormView({
  ...
  const handlePrintReport = async () => {
    if (activePackagingProject?.project_id && activeSession?.session_id) {
      const url = `https://vendor-portal.megaperintis.co.id/qc/print/${activePackagingProject.project_id}/${activeSession.session_id}`;
      if (window && (window as any).__TAURI_INTERNALS__) {
        try {
          const { openUrl } = await import('@tauri-apps/plugin-opener');
          await openUrl(url);
        } catch (e) {
          console.error('Failed to open URL via Tauri opener plugin:', e);
