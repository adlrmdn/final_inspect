...
1. **Presentation** (`src/components/`, `src/App.tsx`) — React 19 components. `App.tsx` is the root, managing view routing (`dashboard` | `form`), the Kaizen AI copilot panel, and voice recognition (Web Speech API). `Dashboard.tsx` shows template cards and a chat interface. `FormView.tsx` renders dynamic forms from template schemas and includes a yield calculator.
